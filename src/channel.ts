 ...
 *      }
 */
function channelDetailsV2(token: string, channelId: number): channelDetails | errorMessage {
  const data = getData();
  ...
  const channelsMemberOf = channelsListV2(token).channels;
  ...
  const channelDetails = data.channels.find(channel => channel.channelId === channelId);
...
export { channelDetailsV2, channelJoinV1, channelInviteV1, channelMessagesV1 };
