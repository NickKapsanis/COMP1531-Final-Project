 //@module channel.test
  ...
  //testing channelsCreateV1 function
 import { channelsCreateV1 } from './channels';

 test('tests if created channel is correctly public/private', () => {
  let CreatedChannelId = channelsCreateV1(1234, Hayden, false);
  expect(CreatedChannelId.isPublic).toEqual(false);

  let CreatedChannelId = channelsCreateV1(4321, Smith, true);
  expect(CreatedChannelId.isPublic).toEqual(true);
 });
 

 test('tests if created channel has correct name and authUserId', () => {
  let CreatedChannelId = channelsCreateV1(6969, Bob, true);
  expect(CreatedChannelId.name).toEqual(Bob);
  expect(CreatedChannelId.authUserId).toEqual(6969);
 });
  ...
  test('tests if correct channel properties are listed in channel list', () => {
    ...
    let fifthCreatedChannel = channelsCreateV1(6969, Rufus, true);
